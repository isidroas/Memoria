...
#    gps_insert_pos = int(GPS_DELAY / DT) - 1
    ...
    if IMAGE_EXTENSION=="pdf":
        results_path = IMAGE_FOLDER + os.sep
    else:
    ...
    if DRAW_ESTIMATED:
        ax.plot(t, p_est[0, :], label="$P_x$ EKF", color="tab:orange")
        if OUTPUT_CORRECTION:
            ax.plot(t, p_output[0, :], label="$P_x$ Salida", color="tab:green")
    ...
    ax.plot(t, p[0, :], label="$P_x$ Groundtruth", color="tab:blue", linestyle="--")
    plt.xlabel("t (s)")
    plt.ylabel("$P_x$ (m)")
    ax.legend()
    ...
    if DRAW_ESTIMATED:
        ax.plot(t, p_est[1, :], label="$P_y$ EKF", color="tab:orange")
        if OUTPUT_CORRECTION:
            ax.plot(t, p_output[1, :], label="$P_y$ Salida", color="tab:green")
            if SHOW_OUTPUT_CORRECTION:
    ...
    ax.plot(t, p[1, :], label="$P_y$ Groundtruth", color="tab:blue", linestyle="--")
    plt.xlabel("t (s)")
    plt.ylabel("$P_y$ (m)")
    ax.legend()
    ...
    ax.set_title("Y vs X")
    if DRAW_ESTIMATED:
        ax.plot(p_est[0, :], p_est[1, :], label="$P$ EKF", color="tab:orange")
        if OUTPUT_CORRECTION:
            ax.plot(p_output[0, :], p_output[1, :], label="$P$ Salida", color="tab:green")
    if SHOW_GPS:
        ax.scatter(gps[0, :], gps[1, :], label="$P$ GPS", marker="x", color="tab:green")
    ax.plot(p[0, :], p[1, :], label="$P$ Groundtruth", color="tab:blue", linestyle="--")
    plt.xlabel("$P_x$ (m)")
    plt.ylabel("$P_y$ (m)")
    ax.legend()
    ax.set_aspect("equal")
    ...
    if DRAW_ESTIMATED:
        ax.plot(t, v_est[0, :], label="$V_x$ EKF", color="tab:orange")
        if OUTPUT_CORRECTION:
            ax.plot(t, v_output[0, :], label="$V_x$ Salida", color="tab:green")
            if SHOW_OUTPUT_CORRECTION:
    ...
    ax.plot(t, v[0, :], label="$V_x$ Groundtruth", color="tab:blue", linestyle="--")
    plt.xlabel("t (s)")
    plt.ylabel("$V$ (m/s)")
    ax.legend()
    ...
    if DRAW_ESTIMATED:
        ax.plot(t, v_est[1, :], label="$V_y$ EKF", color="tab:orange")
        if OUTPUT_CORRECTION:
            ax.plot(t, v_output[1, :], label="$V_y$ Salida", color="tab:green")
            if SHOW_OUTPUT_CORRECTION:
    ...
    ax.plot(t, v[1, :], label="$V_y$ Groundtruth", color="tab:blue", linestyle="--")
    plt.xlabel("t (s)")
    plt.ylabel("$V$ (m/s)")
    ax.legend()
    ...
    ax.plot(
        t, accel_gt[0, :], color="tab:orange", label="$a_x$ Groundtruth", linestyle="--"
    )
    ax.plot(
        t, accel_gt[1, :], color="tab:blue", label="$a_y$ Groundtruth", linestyle="--"
    )
    ax.plot(t, accel[0, :], color="tab:orange", label="$a_x$ measure")
    ...
    # Errors
    p_groundtruth_del = np.empty((2, DATA_L))*np.nan 
    v_groundtruth_del = np.empty((2, DATA_L))*np.nan
    theta_groundtruth_del =  np.empty(DATA_L)*np.nan
    p_groundtruth_del[0,:] = shift(p[0,:], buffer_size, cval=np.NaN)
    p_groundtruth_del[1,:] = shift(p[1,:], buffer_size, cval=np.NaN)
    v_groundtruth_del = shift(v, buffer_size, cval=np.NaN)
    theta_groundtruth_del = shift(v, buffer_size, cval=np.NaN)

    fig, ax = plt.subplots()
    ax.set_title("Error de velocidad")
    ax.plot(t, np.sqrt(P_est[2, 2, :]), color="tab:orange", label=r"$\sigma$ $V_x$",linestyle="--")
    ax.plot(t, np.sqrt(P_est[3, 3, :]), color="tab:blue", label=r"$\sigma$ $V_y$",linestyle="-.")
    if SHOW_OUTPUT_ERROR and OUTPUT_CORRECTION:
        ax.plot(t, abs(v[0, :]-v_output[0, :]), color="tab:orange", label="Error $V_x$ Salida")
        ax.plot(t, abs(v[1, :]-v_output[1, :]), color="tab:blue", label="Error $V_y$ Salida")
    else:
        ax.plot(t, abs(v[0, :]-v_est[0, :]), color="tab:orange", label="Error $V_x$ EKF")
        ax.plot(t, abs(v[1, :]-v_est[1, :]), color="tab:blue", label="Error $V_y$ EKF")
    plt.xlabel("t (s)")
    ...
    fig, ax = plt.subplots()
    ax.set_title("Error de posición en X")
    ax.plot(t, np.sqrt(P_est[0, 0, :]), color="tab:orange", label=r"$\sigma(P_x)$")
    if OUTPUT_CORRECTION:
        if SHOW_OUTPUT_ERROR:
            ax.plot(t, abs(p[0, :]-p_output[0, :]), color="tab:green", label="Error $P_x$ Salida")
        ax.plot(t, abs(p_groundtruth_del[0, :]-p_est[0, :]), color="tab:blue", label="Error $P_x$ EKF")
    else:
        ax.plot(t, abs(p[0, :]-p_est[0, :]), color="tab:blue", label="Error $P_x$ EKF")
    plt.xlabel("t (s)")
    plt.ylabel("error (m)")
    ax.legend()
    plt.savefig(results_path + "Px_error." + IMAGE_EXTENSION)
    

    fig, ax = plt.subplots()
    ax.set_title("Error de posición en Y")
    ax.plot(t, np.sqrt(P_est[1, 1, :]), color="tab:orange", label=r"$\sigma(P_y)$")
    if OUTPUT_CORRECTION:
        if SHOW_OUTPUT_ERROR:
            ax.plot(t, abs(p[1, :]-p_output[1, :]), color="tab:green", label="Error $P_y$ Salida")
        ax.plot(t, abs(p_groundtruth_del[1, :]-p_est[1, :]), color="tab:blue", label="Error $P_y$ EKF")
    else:
        ax.plot(t, abs(p[1, :]-p_est[1, :]), color="tab:blue", label="Error $P_y$ EKF")
    plt.xlabel("t (s)")
    plt.ylabel("error (m)")
    ax.legend()
    plt.savefig(results_path + "Py_error." + IMAGE_EXTENSION)
    
    fig, ax = plt.subplots()
    ax.set_title("Error de inclinación")
    ax.plot(t, np.sqrt(P_est[4, 4, :]), label=r"$\sigma$ $\theta$")
    if SHOW_OUTPUT_ERROR and OUTPUT_CORRECTION:
        ax.plot(t, abs(theta-theta_output), label=r"Error $\theta$ Salida")
    else:
        ax.plot(t, abs(theta-theta_est), label=r"Error $\theta$ EKF")
    plt.xlabel("t (s)")
